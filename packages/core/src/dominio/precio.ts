 ...
 * incluido** (lo que ve el cliente). El costo y el margen ya se ingresan sobre
 * ese precio final: la derivación NO suma el impuesto aparte. En la factura, el
 * impuesto se *extrae* de este precio (ver `factura.ts`).
 */
...
/** Precio final a partir de costo + % de ganancia. */
 ...
 * precio de venta dado y un costo conocido.
 *
...
export function calcularPrecioVenta(input: CalculoPrecioInput): number {
  ...
  return precioBaseDesdeCosto(costo, pctGanancia);
}

/**
 * Sugerencia inicial de `precio_2`/`precio_3` (§ PRECIOS): mismo cálculo que
 * `calcularPrecioVenta` (margen fijo sobre costo), pero con un
