 ...
 * sincronización configurada) o la red falla, no rompe nada — la caja sigue
  ...
  if (!supabaseUrl || !supabaseAnonKey || !syncEmail || !syncPassword) {
    console.warn(
      "Sincronización hacia Supabase desactivada: faltan variables VITE_SUPABASE_URL / " +
        "VITE_SUPABASE_ANON_KEY / VITE_SYNC_EMAIL / VITE_SYNC_PASSWORD (ver packages/desktop/.env.example).",
    );
    return;
  }
