...
export function Personal() {
  ...
  function editar(u: Usuario) {
    setForm({
      ...
      rol: u.rol,
      pin: "",
      overrides: overridesDesdeJson(u.permisos_json),
    });
    setErrores([]);
  }
  ...
  async function guardar() {
    if (!form) return;
    const permisos_json = Object.keys(form.overrides).length
      ? JSON.stringify(form.overrides)
      : null;
    try {
      if (form.id === null) {
  ...
  async function alternarActivo(u: Usuario) {
    try {
      if (u.activo === 1) {
        if (!(await confirmar(`¿Desactivar a "${u.nombre}"?`, { textoConfirmar: "Desactivar" })))
          return;
    ...
    <div>
      ...
      {form && (
        <div style={{ ...s.tarjeta, marginBottom: 16 }}>
          ...
          <div style={{ marginTop: 16 }}>
            <label
              style={{ ...s.label, display: "flex", alignItems: "center", gap: 6, marginTop: 0 }}
            >
              <ShieldCheck size={15} /> Permisos ({ETIQUETA_ROL[form.rol]} por defecto, marca para
              dar una excepción)
            </label>
                  ...
                  >
                    ...
                    {esExcepcion && (
                      <span
                        style={{
                          ...s.badge,
                          background: efectivo ? c.verdeFondo : c.rojoFondo,
                          color: efectivo ? c.verde : c.rojo,
                        }}
                      >
      ...
      {reseteando && (
        <div style={{ ...s.tarjeta, marginBottom: 16 }}>
          ...
          <div style={s.formFooter}>
            <button style={s.boton} onClick={confirmarReseteo}>
              Guardar PIN
            </button>
            <button style={s.botonSecundario} onClick={() => setReseteando(null)}>
              Cancelar (Esc)
            </button>
          </div>
        </div>
              ...
              {lista.length === 0 && (
                <tr>
                  <td style={s.filaVacia} colSpan={5}>
                    Sin personal. Crea el primero con "+ Nuevo usuario".
                  </td>
                </tr>
              )}
              {lista.map((u) => {
                  ...
                  <tr key={u.id}>
                    ...
                    <td style={s.td}>
                      <span
                        style={{
                          ...s.badge,
                          background: u.activo === 1 ? c.verdeFondo : c.grisClaro,
                          color: u.activo === 1 ? c.verde : c.gris,
                        }}
                      >
                    ...
                    <td style={{ ...s.td, whiteSpace: "nowrap" }}>
                      <button style={s.botonSecundario} onClick={() => editar(u)}>
                        Editar
                      </button>{" "}
                      <button style={s.botonSecundario} onClick={() => abrirReseteo(u)}>
                        Resetear PIN
                      </button>{" "}
                      <button
