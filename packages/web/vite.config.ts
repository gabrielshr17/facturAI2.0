...
export default defineConfig(({ mode }) => {
  const modoRemoto = mode === "remoto";
  return {
    base: modoRemoto ? "/facturai/" : "/",
    build: modoRemoto ? { outDir: "../remoto/public/facturai", emptyOutDir: true } : {},
    define: modoRemoto
      ? {
          "import.meta.env.VITE_MODO_REMOTO": JSON.stringify("true"),
          "import.meta.env.VITE_SYNC_EMAIL": JSON.stringify(""),
          "import.meta.env.VITE_SYNC_PASSWORD": JSON.stringify(""),
        }
      : {},
    plugins: [
      react(),
